...
class CanvasOnesignalApi {
	...
	public function send_batch_notification( $data, $tagNames = array(), $user_ids = null ) {
		...
		// get new tags (key and value pair)
		$push_notification_tags = unserialize(Canvas::get_option('push_notification_tags'));
		...
		if ( ! empty( $data['users'] ) ) { // users
		...
		} elseif ( ! empty( $data['tags'] ) || (get_option( 'ml_pnt-push_tags_status') === 'yes' && $push_notification_tags) ) { // tags
			$filters = array();
			if (! empty( $data['tags'] )) {
			...
			// add new tag to filter - start
			if (get_option( 'ml_pnt-push_tags_status') === 'yes' && $push_notification_tags) {
				foreach ($push_notification_tags as $tag_key => $tag_value) {
					if (isset($fields['filters']) && $fields['filters'] && ! empty( $filters )) {

						$filters_tags[] = array( 'operator' => 'OR' );
					} else {
						if ( ! empty( $filters_tags ) ) {
							$filters_tags[] = array( 'operator' => 'OR' );
						}
					}
					$filters_tags[] = array(
						'field'    => 'tag',
						'key'      => strtolower( $tag_key ),
						'relation' => '=',
						'value'    => strtolower( $tag_value ),
					);
				}
				if (isset($fields['filters']) && $fields['filters']) {
					$fields['filters'] = array_merge($fields['filters'], $filters_tags);
				} else {
					$fields['filters'] = $filters_tags;
				}

				// merge new tags and old tags
				$tagNames = array_merge($tagNames, $push_notification_tags);
				$data['tags'] = array_merge($data['tags'], $push_notification_tags);
			}
			// add new tag to filter - end
