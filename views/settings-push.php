...
<?php if (get_option( 'ml_pnt-push_tags_status') === 'yes') { 
	

	$serialized_data = Canvas::get_option('push_notification_tags');
	$push_notification_tags = [];
	// If data exists, unserialize it
	if ($serialized_data) {
		$push_notification_tags = unserialize($serialized_data);
	}
	
	?>
	<!-- Notifications Tags -->
	<div class="cas--settings cas-settings--wide canvas-block">
		<div class="cas--settings__title">Push Notifications Tags </div>
		<div class="cas--settings__content">

			<div class="cas--settings__layout-row">
				<div class="cas--settings__layout-row-item">
					<label for="">
						<span class="cas--label__text"><?php esc_html_e( 'Add Tags', 'canvas' ); ?></span>
					</label>
					<p class="cas--description"><?php esc_html_e( 'Add the key-value pairs for push notification tags.', 'canvas' ); ?></p>
				</div>
					<div class="cas--settings__layout-row-item">

					<?php 
					$remove_button = '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke=""><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill-rule="evenodd" clip-rule="evenodd" d="M19.207 6.207a1 1 0 0 0-1.414-1.414L12 10.586 6.207 4.793a1 1 0 0 0-1.414 1.414L10.586 12l-5.793 5.793a1 1 0 1 0 1.414 1.414L12 13.414l5.793 5.793a1 1 0 0 0 1.414-1.414L13.414 12l5.793-5.793z" fill="#FF0000"></path></g></svg>';
					$add_button = '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M4 12H20M12 4V20" stroke="#2271b1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>';
					
					?>

						<div class="field_wrapper">

								<div class="push-tag-input-header">
									<div class="input-header"><b>Key</b></div>
									<div class="input-header"><b>Value</b></div>
									<div class="blankdiv"></div>
								</div>

								<?php 	if ($push_notification_tags) {
											$count_tag = 1; 
											foreach ($push_notification_tags as $tag_key => $tag) { ?>
												<div class="repeatable-filed">
													<input type="text" placeholder="key" name="push_tag_key[]" value="<?php echo $tag_key; ?>"/>
													<input type="text" placeholder="value" name="push_tag_value[]" value="<?php echo $tag; ?>"/>
													<a class="remove_button button button-large" title="Remove field">
								...
								<?php $count_tag++;	
										...
										<div class="tag-button-outer"><a class="add_button button button-large">
										<?php echo $add_button ?>
										</a></div>
								<?php } else { ?>
									<div class="repeatable-filed">
										<input type="text" placeholder="key" name="push_tag_key[]" value=""/>
										<input type="text" placeholder="value" name="push_tag_value[]" value=""/>
										<a class="add_button button button-large" title="Add field"><?php echo $add_button ?></a>
									</div>
								<?php	}	?>
							</div>
					</div>
					...
					<div class="cas--settings__layout-row-item">
						<?php
							...
							foreach ( $email_templates as $email_key => $email ) :
								...
								$checked = (is_array($woo_email_options)) ? (in_array( $email->id, $woo_email_options, true ) ? 'checked' : '') : '';
...
jQuery(document).ready(function() {
	...
	var fieldHTML = '<div class="repeatable-filed"><input type="text" placeholder="key" name="push_tag_key[]" value=""/><input type="text" placeholder="value" name="push_tag_value[]" value=""/><a class="remove_button button button-large"><?php echo $remove_button ?></a></div>';

	var addButton = '<div class="tag-button-outer"><a class="add_button button button-large"><?php echo $add_button ?></a></div>';
	
	jQuery(document).on('click', '.add_button', function() {

		let lastRepeatableField = document.querySelector('.field_wrapper > .repeatable-filed:nth-last-child(2)');

		if (lastRepeatableField) {
			let keyInput = lastRepeatableField.querySelector('input[name="push_tag_key[]"]');
			let valueInput = lastRepeatableField.querySelector('input[name="push_tag_value[]"]');
			
			let key = keyInput.value.trim();
			let value = valueInput.value.trim();

			if (key !== '' && value !== '') {
				jQuery('.field_wrapper').append(fieldHTML);

				let currentElement = jQuery(this);
				currentElement.removeClass("add_button");
				currentElement.addClass("remove_button");
				currentElement.html('<?php echo $remove_button ?>');
				currentElement.blur();

				if (jQuery('.tag-button-outer').length) {
					jQuery('.tag-button-outer').remove();
				}
				jQuery('.field_wrapper').append(addButton);
			}
		} else {
			let countRepeatable = jQuery('.field_wrapper > .repeatable-filed').length;
			if (countRepeatable == 1) {
				let pushKey = document.querySelector('input[name="push_tag_key[]"]');
				let pushValue = document.querySelector('input[name="push_tag_key[]"]');
				if (pushKey.value.trim() == '' && pushValue.value.trim() == '') {
	...
	jQuery(wrapper).on('click', '.remove_button', function(e){
		e.preventDefault();
		jQuery(this).parent('div').remove();
		let repeatableItems = jQuery('.repeatable-filed').length


		// add field if all input removed
		if (repeatableItems === 0) {

			let newFieldHTML = '<div class="repeatable-filed"><input type="text" placeholder="key" name="push_tag_key[]" value=""/><input type="text" placeholder="value" name="push_tag_value[]" value=""/><a class="add_button button button-large"><?php echo $add_button ?></a></div>';

			jQuery('.field_wrapper').append(newFieldHTML);
